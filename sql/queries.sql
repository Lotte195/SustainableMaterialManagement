...
-- And if we know which coutries are better at recycling on average we can identify what the better strategies are, since there are multiple ones. 
SELECT c.country_name, AVG(rc.recycling_efficiency_rate) AS avg_efficiency -- selects country name and calculates the average recycling efficiiency 
FROM country c -- start from the country table and c becomes the alies 
...
-- The other query that we used was "WHERE NOT EXISTS" to see which if there exists suppliers that do NOT use any extraction method, and therefore have the highest environmental risk rating (5)
-- This is important for our database to recognize companies that don't use the highest risk level, and invest in them, because it has less of a negative effect on the envriontment.
SELECT s.supplier_id, s.supplier_name, c.country_name  -- picks out supplier id, name and country name 
FROM supplier s -- starts from supplier country and s becomes the  alies 
...
WHERE NOT EXISTS (        -- keeps this supplier only if subquery below finds NOTHING 
    ...
    FROM extraction e      -- starts from extraction, e becomes the alies 
...
FROM waste_record wr -- starts from waste_record and wr becomes the alies 
